...
import { useResendOtpMutation, useVerifyOtpMutation } from "../../redux/services/authSlice";
...
export default function VerifyOtp() {
    ...
    const handleSubmit = async () => {
        try {
            const response: any = await verifyOtp({ code: otp, email });
    ...
    const handleResend = async () => {
        ...
        const response: any = await resend(email)
        try {
            if (response?.data?.status) {
                Swal.fire({
                    icon: "success",
    ...
    return (
        <>
            <div className="text-center Bold-heading">Verify OTP</div>
